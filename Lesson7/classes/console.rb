...
class Console
  ...
  def show_all_stations_usage
    @stations.keys.each {|station| puts "Station #{station.name}: #{station.trains_list}"}
  end

  def use_space_in_wagon(how_much)
    wagon_number = ask_for_wagon
    raise "ERROR: cannot find this wagon in the list:#{@wagons.keys}" unless @wagons.include?(wagon_number)
    @wagons[wagon_number].use_capacity(how_much) if @wagons[wagon_number].class == CargoWagon
    @wagons[wagon_number].use_seats if @wagons[wagon_number].class == PassengerWagon
  end
  ...
  def create_wagon_for_train(train_number)
    wagon_number = ask_for_new_wagon_number
    capacity = ask_for_new_wagon_capacity
    wagon = CargoWagon.new(wagon_number, capacity) if @trains[train_number].class == CargoTrain
    wagon = PassengerWagon.new(wagon_number, capacity) if @trains[train_number].class == PassengerTrain
    wagon
  end
